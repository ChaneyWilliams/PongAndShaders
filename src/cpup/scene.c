...
{
    printf("im called\n");
    Entity* yep = (*_scene)->entities;
    if ((*_scene)->entities != NULL)
    {
        printf("im doing stuf");
        int count = vec_count(&(*_scene)->entities);
...
Entity* GetEntity(Scene* scene, int id)
{
    ...
    int count = vec_count(&scene->entities);
    for (int i = 0; i < count; i++)
    {
    ...
    count = vec_count(&scene->startEntities);
    for (int i = 0; i < count; i++)
    {
        if (scene->startEntities[i].id == id)
            return &scene->startEntities[i];
    }
